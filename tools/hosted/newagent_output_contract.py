...
def receipt(value: dict, *, prompt: str | None = None, purpose: str | None = None,
    ...
    require(type(value) is dict and type(value.get("schema_version")) is int
            and value["schema_version"] in (1, 2), "receipt_schema")
    receipt_keys = MANAGED_RECEIPT_KEYS if value["schema_version"] == 2 else RECEIPT_KEYS
    ...
    require(type(request.get("prompt")) is str and request["prompt"].startswith(_PREFIX)
            and request["prompt"].endswith(_SUFFIX), "receipt_chatml")
    original = request["prompt"][len(_PREFIX):-len(_SUFFIX)]
    ...
    expected = {"prompt": _PREFIX + original + _SUFFIX, "n_predict": 8 if value["purpose"] == "warmup" else 64,
    ...
    if value["schema_version"] == 2 and value["backend_execution"] is not None and owner is not None:
...
def agent_result(command: dict, schema: int, *, require_live: bool = False) -> None:
    ...
    if name not in ("agent", "room", "cell", "ask", "resources"):
        return
    valid = schema in (3, 4, 5, 6, 7) and (name == "agent" and len(args) == 1 and args[0] in ("status", "start", "stop", "restart")
            or name == "room" and len(args) == 1 and args[0] in ("status", "discover", "bind", "reconcile")
            or schema in (5, 6, 7) and name == "cell" and len(args) == 1 and args[0] in ("status", "activate", "deactivate")
            or schema in (4, 5, 6, 7) and name == "resources" and len(args) == 1 and args[0] in ("link", "status", "sample")
    ...
    keys(value, RESOURCE_PUBLIC_KEYS if schema in (4, 5, 6, 7) else PUBLIC_KEYS, "reply_keys")
    require(type(value["schema_version"]) is int and value["schema_version"] == {3: 1, 4: 2, 5: 3, 6: 4, 7: 4}[schema]
    ...
    require(value["outcome"] == command["outcome"] and value["outcome"] in ("OK", "ERROR")
            ...
            and (value["error"] is None or value["error"] in ERRORS
    ...
    if action == "ask":
        if inference is None:
            ...
            expected_receipt_keys = MANAGED_RECEIPT_KEYS if schema >= 6 else RECEIPT_KEYS
            require(type(inference) is dict and set(inference) == expected_receipt_keys | REQUEST_CONTEXT
                    and type(inference["schema_version"]) is int and inference["schema_version"] == (2 if schema >= 6 else 1)
...
def render_agent(command: dict) -> str:
    """Render only a previously validated public response."""
    name, args, value = command["name"], command["args"], command["result"]
