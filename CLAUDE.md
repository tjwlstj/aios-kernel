...
the next task. The remaining M3 process path is ordered as process-owned evidence
snapshot → append-only transition event → live continuation/switch; before a
high-risk continuation slice, apply the entry gate in
`docs/tools/verification_tooling_evolution_design_ko.md`.
