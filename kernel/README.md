...
| `core/` | `main.c`, health, acpi, time, shell, `kernel_room.c`, `kernel_room_management.c`, `kernel_room_source_binding.c`, user_mode/user_access, selftest, `linker.ld` |
...
- `CURRENT` native K2-a source binding은 K1 ABI와 분리된 schema 1/256B snapshot으로 Node 101을 producer-owned SLM MAIN source에 결속한다. boot-local immutable oracle이며 live reconcile/apply 경로가 아니다. `core/kernel_room_source_binding.c`.
