...
현재 QEMU 부팅 로그 기준으로 AIOS는 `stable`이 아니라 `degraded`다.

이유:

- `e1000`가 초기화는 되지만 link up이 안 됨
- `TX smoke`도 실패
- 따라서 network I/O 경로는 상용 안정도 기준에서 열화 상태로 간주
...
1. 실제 데이터 경로 부족
- network: RX/TX 실사용 경로 미완
- USB: transfer ring 없음
- storage: read/write 경로 없음
