...
- **스코프 진화:** M2의 최초 ELF 왕복은 공유 부트 page table로 시작했고 M3-b-3b2a에서 정적 private CR3 실행, M3-b-3b2b에서 그 CR3·run state·ring0 entry stack을 묶는 bounded bootstrap process 실행으로 교체했다. 여전히 범용/동적 process table이나 동시 유저 태스크를 뜻하지 않는다.
