...
### 3-0. 현재 전역 작업 — 자기 참조·관측·행동·피드백의 단계적 실증 (2026-09-13)
...
이전 대화·사용자 작업 선택의 자동 문맥은 아직 없다. v0.10의 실행 중 조회·취소는
`PARTIAL`이며 한정 실제 모델 TaskSmoke의 PASS가 아래 작업의 기준선이다. CLI 표시와
실제 모델의 소비는 별도로 증명한다. CPU·RSS만으로 효율 개선을 선언하지 않으며 같은
...
**사용자가 승인한 장기 목표와 반복 절차:**

AI가 자기 작업 공간·상태·사용 가능한 행동을 근거로 이해하고, 행동 전 예상과 실제 결과의
차이를 다음 판단에 반영하며, 사용자와 지속적으로 활동하는 원래 구상을 실제로 검증한다.
전용 모델을 곧바로 확보할 수 있다는 전제는 두지 않는다. 고정 모델의 prompt·외부 기록과
이후 adapter·가중치 적응을 구분해 연구하며, 작은 fixture나 단발 pilot으로 전체 목표를
완료 처리하지 않는다. 연구 가설의 기각도 결과이지만 미구현 기능의 완료 증거는 아니다.

작업은 **구현·실제 증거 확보 → 검증된 beta 게시 → 사용자가 지정한 기존 GPT 연구
대화의 연구·검토 → 로컬 근거 대조·채택/보류/기각 → 다음 실험**으로 반복한다.
GPT의 제안은 검토 입력이며 실행 판정이나 정본 변경 권위가 아니다. 사전 조사·모델 적응
연구는 구현과 병행할 수 있고, 게시된 결과의 검토는 해당 beta와 보존 증거를 기준으로 한다.
자동 일정이나 새 대화 생성을 이 문서가 약속하지 않는다. 자세한 실험·연구 기록 절차는
[자기 참조 연구·실증 가이드](self_reference_research_workflow_ko.md)가 소유하며,
우선순위와 아래 전체 완료 기준은 이 전역 큐에서만 정한다.

<a id="self-reference-acceptance"></a>

**장기 목표의 다섯 단계와 완료 판정:**

1. **자기 참조와 사실 근거:** 안정적인 에이전트 식별과 매 실행의 epoch를 분리하고
   Task·관측·예측·행동·결과를 연결한다. 측정값·추정·STALE·UNKNOWN을 구별하며 자기
   설명이 사실이나 권한의 원천이 되지 않는다. 실제 실행 기록으로 연결의 정확성을
   확인해야 하며 persona 문구나 ID 필드 추가만으로 통과하지 않는다.
2. **실제 행동과 피드백에 따른 수정:** 정상 상황뿐 아니라 대상 version 변경,
   소유/권한 철회, 관측 중단을 실제로 가한다. 실행 전 예측 → 제한된 typed action →
   독립 결과 관측 → 자기 상태 추정 수정 → 다음 실제 행동을 연결한다. 같은 사실의
   비교 조건에서 불가능한 행동 재시도, 오래된 능력 주장, 유효한 대안, 작업 완료와 비용을
   측정한다. private sandbox pilot 뒤에도 실제 AIOS 증거를 읽어 보고하는 유용한 Task와
   Linux Task 경로에 연결한 반복 실증이 필요하며 fixture만으로 완료하지 않는다.
3. **연속성과 사용자 통제:** CLI·모델·runtime의 재시작 또는 교체에 새 epoch와
   재관측을 적용한다. 완료·미완료·UNKNOWN을 보존하고 중복 효과나 철회된 권한의 부활을
   막는다. 실제 사용자 취소, 기억 삭제, 관측 구독 해지가 반영되는지 확인한다.
   저장된 기록이 있다고 자동 재개나 지속적 자아를 입증한 것으로 보지 않는다.
4. **공간 연결과 로컬 효율:** 실제 작업·하드웨어에서 같은 사실의 flat/관계 표현,
   정적 경로/경로 cache/학습된 경로, 고정 배치/제한된 자원 조절을 구분해 비교한다.
   보류한 평가 시나리오에서 같은 정확도 목표를 유지한 비용·지연·관측 부담의 개선과
   회귀를 측정한다. prompt·외부 기억·adapter·가중치 변경을 각각 분리하고 학습이 있으면
   데이터·가중치 계보와 재현 조건을 보존한다. 반복 사용이나 QEMU 시간만으로 학습·효율을
   선언하지 않으며 연구 결과에 따라 유효한 방식만 제품 단계로 옮긴다.
5. **AIOS 책임과 실제 통합 사용:** 자체 native AIOS가 실제로 소유·강제하는 최소
   실행·자원·객체·권한 책임을 명시하고 Linux userspace·드라이버의 역할과 분리해
   입증한다. 실제 제품에서 관측 → 판단 → 행동 → 실패/복구의 유용한 작업을 지속적으로
   수행하는 증거가 필요하다. hosted 성공만으로 독립 native 시스템이라는 전체 목표를
   완료 처리하지 않는다.

이 단계들은 넓은 원래 목표를 보존하는 수락 기준이다. 첫 실험은 아래 한정 범위만
평가하며, 의식·주관적 경험·인격의 증명은 어느 단계의 판정에도 포함하지 않는다.
단계별 성공·실패·미확인은 해당 증거의 범위로 기록하고 기술 K/H/M/C/W축은 별도로 판정한다.

**최근 완료한 범위와 지금 수행할 순서:**

1. **기준선 — v0.10 실제 모델 TaskSmoke의 한정 PASS, 제품 성숙도 `PARTIAL`:**
   같은 CLI의 정상 모델 답변 1회와 별도 진행 중 Task 취소·worker 종료·전체 backend
   독립 종료 관측, DNS/HTTPS와 정상 VM 종료를 확인했다. source별 Windows 전체·Linux
...
3. **pilot 이후 실제 사용 증거와 연구 반영:** pilot의 보존 결과를 beta와 기존 GPT
   연구 대화에서 검토하고 로컬 독립 판정과 대조한다. 그 결과를 바탕으로 실제 AIOS
   증거를 읽고 보고하는 Task, Linux Task 통합, 연속성·효율의 남은 반례를 단계적으로
   실증한다. prompt/외부 기억과 adapter/가중치 적응의 필요성은 같은 과제의 근거로
   판단하며 모델 교체 자체를 성과로 보지 않는다.
4. **운영 이미지의 미완료 의존성 유지:** source 39개 이미지의 backend 시작 주체와
   같은 CLI의 lease/child handle 보유 범위를 확인해야 한다. 기존 image35/36의
   시작·정상 종료 계약을 보존하고 이미지 Task 경로를 정한다. 새 이미지 생성·두 부팅·
   실제 모델 Task acceptance는 미완료이며 외부 init의 backend를 CLI 소유로 자동
   간주하지 않는다. 이 이미지 작업만으로 위 장기 목표를 대신하지 않는다.

sandbox 밖의 새 관리·특권 행동은 K5 principal/ownership/authorize와 해당 action·rollback
증거를 먼저 갖춘다. private sandbox의 좁은 응용 검사를 그 정본이나 OS 보안 경계로
승격하지 않으며, 연구가 권한 검사를 바꾸거나 우회하지 못하게 한다.
