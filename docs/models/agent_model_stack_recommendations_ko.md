...
> 문서 수명주기: `REVIEW` — 과거 모델 스택 제안을 보존한 검토 자료이며 현재 모델 선택 정본이 아니다.
> 분류·내용 검토일: **2026-09-09**. 아래 원문의 모델 버전·성능·라이선스·하드웨어 추천과
> 외부 출처의 최신성은 이번 정비에서 재검증하지 않았다. 기재된 기준일이 각 출처의 실제 확인일을
> 뜻하는지도 확인되지 않았으므로, 새 모델 선택·구매·배포의 근거로 바로 사용하지 않는다.
>
> 여러 SLM/LLM의 역할 분담, memory·adapter·학습 전략은 당시 제안 범위다.
> 현재 구현되었거나 실제 모델 간 비교로 효율이 입증된 구성이라는 뜻이 아니다.
> 분류일을 모델 조사일 또는 추천 갱신일로 해석하지 않는다.

원문 기재 기준일: 2026-03-28 (보존)

현재 목적과 성공 조건은 [AIOS 제품 방향 정본](../meta/aios_product_direction_ko.md),
다음 작업은 [전역 작업 큐](../meta/minimal_io_and_maturity_workflow_ko.md)를 따른다.
실제로 실행한 모델·backend와 그 제한은 [backend 수명 가이드](../os/aios_backend_lifecycle_guide_ko.md) 및
[운영 이미지 가이드](../os/aios_operating_image_guide_ko.md)의 보존된 증거를 확인한다.
문서의 재검토 범위는 [문서 최신성 원장](../meta/document_freshness_registry_ko.md)을 따른다.
