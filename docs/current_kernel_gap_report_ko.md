...
### 4) 스케줄러가 실제 타이머 인터럽트와 미연결

스케줄러 자체는 구조가 잘 잡혀 있지만, `ai_sched_tick()`이 실제 하드웨어 주기와 연결되지 않았다.
...
- 하지만 저장소 내에서 tick 함수가 실제 IRQ 경로에서 호출되지 않는다.
- IDT는 예외 처리 중심이고 timer IRQ 계층이 없다.
...
- `timer IRQ -> ai_sched_tick -> context switch`가 커널 core milestone이다.
...
- e1000은 TX smoke까지 간다.
- USB는 xHCI capability probe 수준이다.
- storage는 IDE channel live/status 수준이다.
- 실제 RX, read/write, transfer ring, completion path는 부족하다.
...
- `e1000 RX -> storage read -> xHCI transfer`가 현재 드라이버 1순위다.
