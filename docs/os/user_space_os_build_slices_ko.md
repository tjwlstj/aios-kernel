...
> OLD (2026-07-03): 이 문서는 M1 uaccess/SMAP, M2 static ELF64 loader 완료 이전 기준이다.
...
- `os/tools/evaluate_nodebit_policy.py`와 `os/examples/nodebit_catalog.sample.json`은 userspace policy broker의 runtime overlay NodeBit 판정 규칙과 risky gate를 host-side smoke로 고정한다
...
- `observer service`
- `promotion policy`
- NodeBit 기반 policy broker manifest / smoke
