...
> **REVIEW (2026-08-21):** 초기 코드 경계 판단을 보존하는 참고 문서다. 현재 파일
> 배치와 의존 방향은 [PROJECT.md](../../PROJECT.md), 작업 분류와 문서 관리는
> [통합 작업 진입 가이드](../meta/integrated_work_guide_ko.md)를 우선한다.
...
그리고 2026-04-18 기준 `Kernel Room Topology` 관련 문서는
`docs/kernel-room/` 아래에서 따로 관리한다.
즉, 이 문서는 전체 저장소의 경계 기준을 다루고,
`kernel-room/` 쪽은 그 중 Kernel Room 관련 규칙을 전담한다.
