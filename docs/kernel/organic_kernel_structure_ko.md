...
- `sched/ai_sched.c`는 PIT IRQ0 기반 tick/accounting에는 연결됐지만 아직 실행 컨텍스트를 전환하는 루프는 아니다
...
### 4. timer IRQ accounting -> scheduler preemption

PIT IRQ0에서 `ai_sched_tick()`까지의 bootstrap은 생겼다.
다음 핵심은 tick 이후 runnable task를 선택하고 실제 context switch까지 이어지는 선점 경로다.
