...
3. 현재 브랜치·작업 트리·문서·구현 증거를 먼저 확인한다.
4. 최소 수직 조각을 구현하거나, 조사 전용 요청이면 변경 없이 멈춘다.
5. 변경 위험에 맞는 검증을 수행하고 실행하지 않은 lane을 기록한다.
6. 게시 요청이면 `beta`를 먼저 검증한 뒤 승인된 경우에만 같은 SHA로
